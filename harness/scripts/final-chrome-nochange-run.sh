...
EXPLICIT_PUBLIC_ORIGIN_NOCHANGE_URL_SET="${PUBLIC_ORIGIN_NOCHANGE_URL+x}"
EXPLICIT_PUBLIC_ORIGIN_NOCHANGE_URL="${PUBLIC_ORIGIN_NOCHANGE_URL:-}"
EXPLICIT_PUBLIC_ORIGIN_BOOTSTRAP_URL_SET="${PUBLIC_ORIGIN_BOOTSTRAP_URL+x}"
EXPLICIT_PUBLIC_ORIGIN_BOOTSTRAP_URL="${PUBLIC_ORIGIN_BOOTSTRAP_URL:-}"
EXPLICIT_CONTROLLED_PUBLIC_EXPECTED_REQUESTS_SET="${CONTROLLED_PUBLIC_EXPECTED_REQUESTS+x}"
EXPLICIT_CONTROLLED_PUBLIC_EXPECTED_REQUESTS="${CONTROLLED_PUBLIC_EXPECTED_REQUESTS:-}"
EXPLICIT_CHROME_RUNNER_SET="${CHROME_RUNNER+x}"
EXPLICIT_CHROME_RUNNER="${CHROME_RUNNER:-}"
EXPLICIT_CHROME_HOLD_SECONDS_SET="${CHROME_HOLD_SECONDS+x}"
EXPLICIT_CHROME_HOLD_SECONDS="${CHROME_HOLD_SECONDS:-}"
if [[ -f "$CONFIG_FILE" ]]; then
  load_env_if_present "$CONFIG_FILE"
fi
if [[ -n "$EXPLICIT_PUBLIC_ORIGIN_NOCHANGE_URL_SET" ]]; then
  PUBLIC_ORIGIN_NOCHANGE_URL="$EXPLICIT_PUBLIC_ORIGIN_NOCHANGE_URL"
fi
if [[ -n "$EXPLICIT_PUBLIC_ORIGIN_BOOTSTRAP_URL_SET" ]]; then
  PUBLIC_ORIGIN_BOOTSTRAP_URL="$EXPLICIT_PUBLIC_ORIGIN_BOOTSTRAP_URL"
fi
if [[ -n "$EXPLICIT_CONTROLLED_PUBLIC_EXPECTED_REQUESTS_SET" ]]; then
  CONTROLLED_PUBLIC_EXPECTED_REQUESTS="$EXPLICIT_CONTROLLED_PUBLIC_EXPECTED_REQUESTS"
fi
if [[ -n "$EXPLICIT_CHROME_RUNNER_SET" ]]; then
  CHROME_RUNNER="$EXPLICIT_CHROME_RUNNER"
fi
if [[ -n "$EXPLICIT_CHROME_HOLD_SECONDS_SET" ]]; then
  CHROME_HOLD_SECONDS="$EXPLICIT_CHROME_HOLD_SECONDS"
fi
...
BOOTSTRAP_URL="${PUBLIC_ORIGIN_BOOTSTRAP_URL:-${PUBLIC_ORIGIN_URL:-$NOCHANGE_URL}}"
...
print_kv "chrome_hold_seconds" "$CHROME_HOLD_SECONDS"
