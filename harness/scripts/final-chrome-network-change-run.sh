...
USER_NETWORK_CHANGE_READY_EXPR_SET=0
USER_NETWORK_CHANGE_READY_EXPR=""
if [[ ${NETWORK_CHANGE_READY_EXPR+x} ]]; then
  USER_NETWORK_CHANGE_READY_EXPR_SET=1
  USER_NETWORK_CHANGE_READY_EXPR="$NETWORK_CHANGE_READY_EXPR"
fi
USER_NETWORK_CHANGE_READY_TIMEOUT_SECONDS_SET=0
USER_NETWORK_CHANGE_READY_TIMEOUT_SECONDS=""
if [[ ${NETWORK_CHANGE_READY_TIMEOUT_SECONDS+x} ]]; then
  USER_NETWORK_CHANGE_READY_TIMEOUT_SECONDS_SET=1
  USER_NETWORK_CHANGE_READY_TIMEOUT_SECONDS="$NETWORK_CHANGE_READY_TIMEOUT_SECONDS"
fi
USER_NETWORK_CHANGE_READY_POLL_INTERVAL_MS_SET=0
USER_NETWORK_CHANGE_READY_POLL_INTERVAL_MS=""
if [[ ${NETWORK_CHANGE_READY_POLL_INTERVAL_MS+x} ]]; then
  USER_NETWORK_CHANGE_READY_POLL_INTERVAL_MS_SET=1
  USER_NETWORK_CHANGE_READY_POLL_INTERVAL_MS="$NETWORK_CHANGE_READY_POLL_INTERVAL_MS"
fi
...
if [[ "$USER_NETWORK_CHANGE_READY_EXPR_SET" == "1" ]]; then
  NETWORK_CHANGE_READY_EXPR="$USER_NETWORK_CHANGE_READY_EXPR"
fi
if [[ "$USER_NETWORK_CHANGE_READY_TIMEOUT_SECONDS_SET" == "1" ]]; then
  NETWORK_CHANGE_READY_TIMEOUT_SECONDS="$USER_NETWORK_CHANGE_READY_TIMEOUT_SECONDS"
fi
if [[ "$USER_NETWORK_CHANGE_READY_POLL_INTERVAL_MS_SET" == "1" ]]; then
  NETWORK_CHANGE_READY_POLL_INTERVAL_MS="$USER_NETWORK_CHANGE_READY_POLL_INTERVAL_MS"
fi
...
NETWORK_CHANGE_URL="${PUBLIC_ORIGIN_NETWORK_CHANGE_URL:-${PUBLIC_ORIGIN_URL:-}}"
BOOTSTRAP_URL="${PUBLIC_ORIGIN_BOOTSTRAP_URL:-${PUBLIC_ORIGIN_URL:-$NETWORK_CHANGE_URL}}"
...
print_kv "network_change_url" "$NETWORK_CHANGE_URL"
...
if [[ "$CONFIG_READY" == "1" && "$BASELINE_READY" == "1" ]]; then
  if run_step "network_change_execution" \
    env \
      ...
      PUBLIC_ORIGIN_URL="$NETWORK_CHANGE_URL" \
