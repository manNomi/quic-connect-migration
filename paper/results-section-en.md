...
We then extended this into a transient return-path outage duration sweep. Both A-side and B-side server-to-client packet drops were bounded to 250 ms, 1500 ms, 3000 ms, 6000 ms, and 9000 ms after the proxy switch. For 1 MiB downlink and upload workloads, the 250 ms, 1500 ms, and 3000 ms windows passed in all six rows, while the 6000 ms and 9000 ms windows failed in all four rows. Successful rows reported DOM completion times between roughly 7.5 s and 10.5 s, while failed rows reported DOM error timing between roughly 6.9 s and 11.1 s. Since all rows still recorded proxy switch and qlog HTTP/3/path evidence, local browser workload continuity should be interpreted as a function of outage duration, retransmission opportunity, and browser task timing, not merely as the presence or absence of path frames.
