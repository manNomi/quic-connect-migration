...
이 해석을 검증하기 위해 retry를 끈 wait-only control을 같은 6000ms/9000ms outage와 같은 긴 hold/grace 조건에서 실행했다. 이 대조군은 6/6 FAIL이었고 DOM error timing은 6.923-6.935초로 모였다. 모든 row가 qlog H3/path evidence와 Chrome target QUIC session 1개를 남겼지만 application completion은 false였다. 따라서 downlink retry-enabled PASS를 단순히 “더 오래 기다려서 성공”한 것으로 환원할 수 없으며, application recovery/timer behavior 자체를 별도 실험 축으로 보고해야 한다.

Dashboard/polling형 workload를 보기 위해 250ms/1500ms/3000ms outage에서 `GET /browser-poll` 이후 6회 `/poll`을 반복하는 control도 실행했다. 세 window 모두 3/3 PASS였고 각 row는 server request 7개를 남겼다. 그러나 모든 row가 server remote addr count 2와 Chrome target QUIC session count 2로 분류됐고 qlog PATH_CHALLENGE/PATH_RESPONSE count는 0/0이었다. 따라서 dashboard refresh류 작업은 짧은 outage에서 사용자에게는 연속적으로 보일 수 있지만, 그 메커니즘은 single-session browser CM이 아니라 repeated fetch가 replacement/multiple sessions로 완료된 경우일 수 있다. 이 결과는 대시보드 복구 시간을 평가 기준으로 사용할 때 반드시 session attribution과 함께 보고해야 함을 보여준다.
