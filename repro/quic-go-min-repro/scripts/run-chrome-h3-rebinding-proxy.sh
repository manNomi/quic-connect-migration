...
case "$WORKLOAD" in
  ...
  upload)
    ...
    UPLOAD_COMPLETION_GRACE_MS="${UPLOAD_COMPLETION_GRACE_MS:-$((UPLOAD_DURATION_MS + 1000))}"
    COMPLETION_GRACE="${COMPLETION_GRACE:-${UPLOAD_COMPLETION_GRACE_MS}ms}"
    REQUEST_PATH="${REQUEST_PATH:-/browser-upload?duration_ms=${UPLOAD_DURATION_MS}&chunks=${UPLOAD_CHUNKS}&bytes=${UPLOAD_BYTES}&label=chrome-rebinding-upload}"
    EXPECTED_REQUESTS="${EXPECTED_REQUESTS:-2}"
    ;;
  *)
