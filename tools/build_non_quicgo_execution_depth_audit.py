...
WHY_NOT_QUIC_GO_DEPTH = {
    ...
    "MsQuic": "Selected NAT rebinding and path-validation tests passed, but direct application-triggered active migration and QUIC-aware LB assumptions still need a focused API/deployment check.",
    ...
    "XQUIC": "The NAT rebinding demo passed, but the full suite was deferred to Linux or compiler-flag follow-up because the macOS build path hit unrelated toolchain friction.",
    ...
    "mvfst": "Source/test coverage is strong, but local build/test execution is gated by Buck/getdeps/disk/toolchain cost.",
...
NEXT_NON_IPHONE_GATE = {
    ...
    "MsQuic": "Audit the public API path for controlled migration and pair it with a QUIC-aware LB deployment assumption check.",
    ...
    "XQUIC": "Rerun the full test suite on Linux with compatible compiler flags.",
    ...
    "mvfst": "Use a Linux builder with Buck/getdeps and enough disk, then run the focused migration test targets.",
