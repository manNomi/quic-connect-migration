...
DEPTH_BY_STATUS = {
    ...
    "source_policy_audit_20260701": "client_policy_source_plus_local_baseline",
    "source_inspected": "source_test_map_only",
    "source_edge_boundary_audit_20260701": "managed_or_external_deployment_gate",
...
WHY_NOT_QUIC_GO_DEPTH = {
    ...
    "MsQuic": "Selected v4/v6 NAT rebinding and path-validation runtime tests now have a dedicated fail-closed packet, and the focused API audit still shows constrained local-address control plus a QUIC-aware LB boundary rather than quic-go-style AddPath/Probe/Switch control.",
    "Quinn": "Rust test evidence plus the endpoint-rebind runtime packet are useful for maturity comparison, but the active migration surface remains endpoint-wide rebind rather than quic-go's per-connection AddPath/Probe/Switch API.",
    "Neqo": "Mozilla-adjacent tests and the Firefox/Neqo boundary audit provide broad migration maturity evidence, but this did not become a Firefox browser runtime proof without Firefox/Necko controlled handover rows.",
    ...
    "Chromium Chrome Cronet": "Source policy hooks, tests, NetLog migration events, and Cronet default-disable evidence exist, but actual browser handover behavior is runtime-policy dependent and must be proven with browser rows.",
    "AWS CloudFront": "CloudFront official docs support viewer-edge HTTP/3 Connection Migration, but CloudFront cannot be treated as end-to-end origin Connection Migration without separate origin evidence.",
...
NEXT_NON_IPHONE_GATE = {
    ...
    "MsQuic": "Use the MsQuic rebind/path-validation packet as selected runtime-test evidence; build a separate payload-continuity harness or live QUIC-aware LB row only if reviewers require it.",
    "Quinn": "Use the Quinn rebind runtime packet as Rust-stack endpoint-rebind evidence; add a custom HTTP/3 workload only if reviewers require application-layer Rust evidence.",
    "Neqo": "Use the Firefox/Neqo browser-boundary audit now; run Firefox/Necko controlled rows only when browser-runtime evidence becomes reviewer-critical.",
    ...
    "Chromium Chrome Cronet": "Use the policy-boundary audit now; run Android/Cronet or desktop Chrome active network-change rows when a non-iPhone secondary path is available.",
    "AWS CloudFront": "Use the CDN edge boundary audit now; run a viewer-edge continuity experiment only if the paper needs managed-edge runtime evidence.",
