...
EVIDENCE_ITEMS = [
    ...
    EvidenceItem(
        id="quicly-full-e2e-linux-audit",
        chapter="1",
        category="implementation-runner-readiness",
        implementation="quicly",
        evidence_doc="docs/results/quicly-full-e2e-linux-audit-20260701.md",
        runner_or_tool="harness/scripts/run-quicly-full-e2e-linux.sh",
        local_artifact_id="quicly-full-e2e-linux-local-blocked-20260701",
        claim_strength="focused_positive_with_linux_full_e2e_gate",
        supports="quicly source evidence, focused path-migration e2e PASS, CID sequence 1 first path probe evidence, and a fail-closed Linux full-e2e replay runner.",
        do_not_claim="quicly full `t/e2e.t` PASS, production H2O deployment continuity, browser handover, or quic-go-equivalent public active migration control.",
        next_gap="Run the Linux full-e2e runner and accept full-e2e promotion only when validation=ok_full_e2e.",
    ),
    EvidenceItem(
        id="xquic-full-suite-linux-audit",
        chapter="1",
        category="implementation-runner-readiness",
        implementation="XQUIC",
        evidence_doc="docs/results/xquic-full-suite-linux-audit-20260701.md",
        runner_or_tool="harness/scripts/run-xquic-full-suite-linux.sh",
        local_artifact_id="xquic-full-suite-linux-local-blocked-20260701",
        claim_strength="focused_positive_with_linux_replay_gate",
        supports="XQUIC source callbacks, NAT rebinding validation, registered client/server demo hooks, local NAT rebinding demo PASS, and a fail-closed Linux full-suite replay runner.",
        do_not_claim="XQUIC full test-suite PASS, browser/mobile handover success, or production deployment continuity.",
        next_gap="Run the Linux replay runner on a compatible host and accept only validation=ok with zero failed unit/case markers.",
    ),
    EvidenceItem(
    ...
    EvidenceItem(
        id="mvfst-focused-linux-runner-audit",
        chapter="1",
        category="implementation-runner-readiness",
        implementation="mvfst",
        evidence_doc="docs/results/mvfst-focused-linux-runner-audit-20260701.md",
        runner_or_tool="harness/scripts/run-mvfst-focused-migration-tests-linux.sh",
        local_artifact_id="mvfst-focused-migration-linux-local-blocked-20260701",
        claim_strength="source_test_map_with_linux_runner_gate",
        supports="mvfst dedicated path manager, client active migration, server passive migration/NAT rebinding source evidence, focused BUCK target map, 106 observed test cases, and a fail-closed Linux runner.",
        do_not_claim="Local mvfst build/test PASS, browser handover, production Meta deployment behavior, or quic-go-equivalent active control.",
        next_gap="Run the focused Linux runner with buck2 and enough disk; accept only validation=ok with all three BUCK targets exiting 0.",
    ),
    EvidenceItem(
    ...
    EvidenceItem(
        id="chromium-cronet-policy-boundary-audit",
        chapter="1",
        category="browser-policy-boundary",
        implementation="Chromium/Cronet",
        evidence_doc="docs/results/chromium-cronet-policy-boundary-audit-20260701.md",
        runner_or_tool="tools/build_chromium_cronet_policy_boundary_audit.py",
        local_artifact_id="chromium-cronet-policy-boundary-audit-20260701",
        claim_strength="source_linked_policy_boundary",
        supports="Chromium has client migration hooks and NetLog migration event taxonomy, while Cronet's inspected QUIC path explicitly disables network-change migration by default.",
        do_not_claim="Chrome browser, Android platform HTTP, or Cronet embedded-app live single-session handover success.",
        next_gap="Run Chrome/Cronet active network-change rows with target-session NetLog trigger/success/failure, client path-change, server/qlog, and workload-completion evidence.",
    ),
    EvidenceItem(
        id="firefox-neqo-browser-boundary-audit",
        chapter="1",
        category="browser-runtime-boundary",
        implementation="Firefox + Neqo",
        evidence_doc="docs/results/firefox-neqo-browser-boundary-audit-20260701.md",
        runner_or_tool="tools/build_firefox_neqo_browser_boundary_audit.py",
        local_artifact_id="firefox-neqo-browser-boundary-audit-20260701",
        claim_strength="source_linked_browser_adjacency_boundary",
        supports="Neqo is Firefox-adjacent and has explicit migration API/source support, rebinding handling, preferred-address coverage, qlog transport-parameter observability, and fresh local migration test evidence.",
        do_not_claim="Firefox desktop/mobile live single-session HTTP/3 handover, browser workload continuity, or Firefox runtime policy success.",
        next_gap="Run Firefox desktop against a controlled H3 origin or local neqo-server with Firefox/Necko logging plus server qlog/tuple evidence.",
    ),
    EvidenceItem(
        id="firefox-desktop-runtime-trial-packet",
        chapter="5/7",
        category="browser-runtime-trial-packet",
        implementation="Firefox desktop + Neqo/controlled public H3",
        evidence_doc="docs/results/firefox-desktop-runtime-trial-packet-20260701.md",
        runner_or_tool="tools/build_firefox_desktop_runtime_trial_packet.py",
        local_artifact_id="firefox-desktop-runtime-trial-packet-20260701",
        claim_strength="fail_closed_browser_runtime_packet",
        supports="Firefox desktop runtime promotion is now framed as a reproducible gate: local Neqo H3 baseline, controlled-public H3 baseline, range active, and upload active rows define the required artifacts and acceptance boundaries.",
        do_not_claim="Firefox runtime Connection Migration success, Firefox single-session handover, or Chrome-equivalent NetLog proof.",
        next_gap="Install Firefox/geckodriver or run a manual Firefox profile, open a non-iPhone desktop path-change gate, then execute the local Neqo H3 baseline first.",
    ),
    EvidenceItem(
        id="cdn-edge-cm-boundary-audit",
        chapter="4",
        category="cdn-edge-boundary",
        implementation="AWS CloudFront and Cloudflare managed edge",
        evidence_doc="docs/results/cdn-edge-cm-boundary-audit-20260701.md",
        runner_or_tool="tools/build_cdn_edge_cm_boundary_audit.py",
        local_artifact_id="cdn-edge-cm-boundary-audit-20260701",
        claim_strength="official_doc_edge_boundary",
        supports="Official CDN documentation supports viewer-edge HTTP/3 continuity boundaries and prevents treating managed CDN H3 toggles as origin end-to-end QUIC Connection Migration.",
        do_not_claim="Origin end-to-end QUIC CM, origin qlog path validation, backend same-session continuity, or CloudFront/Cloudflare live handover success.",
        next_gap="Run a managed edge runtime experiment only if the paper needs viewer-edge continuity evidence; use direct-origin or CID-aware LB paths for origin end-to-end CM.",
    ),
    EvidenceItem(
    ...
    EvidenceItem(
        ...
        supports="MsQuic exposes MigrationEnabled, address-change events, constrained QUIC_PARAM_CONN_LOCAL_ADDRESS control, NAT rebinding tests, a companion selected v4/v6 rebind/path-validation runtime-test PASS artifact, and a documented QUIC-aware load-balancing boundary.",
        do_not_claim="quic-go-style AddPath/Probe/Switch control, managed-LB continuity, browser handover success, or runtime payload continuity beyond existing MsQuic tests.",
        next_gap="Use the companion selected runtime-test packet for MsQuic implementation evidence; build payload-continuity or live LB rows only if reviewers require them.",
    ),
    EvidenceItem(
        id="msquic-rebind-pathvalidation-packet",
        chapter="1",
        category="implementation-runtime-positive-control",
        implementation="MsQuic",
        evidence_doc="docs/results/msquic-rebind-pathvalidation-packet-20260701.md",
        runner_or_tool="harness/scripts/run-msquic-rebind-pathvalidation-demo.sh",
        local_artifact_id="msquic-rebind-pathvalidation-local-20260701",
        claim_strength="selected_runtime_test_positive",
        supports="MsQuic selected user-mode runtime-test runner passed v4 and v6 RebindPort, RebindAddr, PathValidationTimeout, and PathValidationLastPathClose: total_ok_count=8, passed_summary_count=2, failed_marker_count=0.",
        do_not_claim="HTTP/3 application continuity, Chrome/Safari/Android browser handover, AWS NLB/CloudFront deployment success, or quic-go-equivalent per-connection active-control behavior.",
        next_gap="Add a payload-continuity harness or live QUIC-aware LB experiment only if reviewers require application/deployment evidence.",
    ),
    EvidenceItem(
        id="ngtcp2-migration-api-boundary-audit",
        chapter="1",
        category="implementation-api-boundary",
        implementation="ngtcp2",
        evidence_doc="docs/results/ngtcp2-migration-api-boundary-audit-20260701.md",
        runner_or_tool="tools/build_ngtcp2_migration_api_boundary_audit.py",
        local_artifact_id="ngtcp2-migration-api-boundary-audit-20260701",
        claim_strength="source_linked_api_boundary",
        supports="ngtcp2 exposes public immediate and validation-gated client migration APIs, path-validation callbacks, disable-active-migration and preferred-address policy handling, qlog observability, example controls, and focused local migration/path-validation test evidence.",
        do_not_claim="Chrome/Safari/Android browser handover, managed-CDN/LB continuity, or production application continuity from source/API evidence alone.",
        next_gap="Use the companion ngtcp2 runtime trial packet for local HTTP/3 example evidence; keep browser and deployment gates separate.",
    ),
    EvidenceItem(
        id="ngtcp2-runtime-trial-packet",
        chapter="1",
        category="implementation-runtime-positive-control",
        implementation="ngtcp2",
        evidence_doc="docs/results/ngtcp2-runtime-trial-packet-20260701.md",
        runner_or_tool="harness/scripts/run-ngtcp2-example-migration-demo.sh",
        local_artifact_id="ngtcp2-example-migration-demo-local-20260701",
        claim_strength="local_runtime_positive",
        supports="The official ngtcp2 osslclient/osslserver HTTP/3 example completed a local migration runtime row: client_exit=0, client local address changed once, qlog counts were present on both sides, and PATH_CHALLENGE/PATH_RESPONSE counters were nonzero.",
        do_not_claim="Chrome/Safari/Android browser handover, managed deployment success, production application continuity, or quic-go-equivalent custom active-control API behavior.",
        next_gap="Use as a second C-library runtime positive control; repeat on a clean host only if reviewers require independent replication.",
    ),
    EvidenceItem(
        ...
        claim_strength="source_linked_api_boundary",
        supports="Quinn exposes server migration policy, endpoint-wide socket rebind, preferred-address support, path-validation machinery, frame stats, fresh local migration/rebind tests, and a companion endpoint-rebind runtime PASS artifact.",
        do_not_claim="quic-go-style per-connection AddPath/Probe/Switch control, HTTP/3 browser workload continuity, managed deployment continuity, or production app continuity.",
        next_gap="Use the companion Quinn runtime packet for endpoint-rebind runtime evidence; keep HTTP/3/browser/deployment rows separate.",
    ),
    EvidenceItem(
        id="quinn-rebind-runtime-packet",
        chapter="1",
        category="implementation-runtime-positive-control",
        implementation="Quinn",
        evidence_doc="docs/results/quinn-rebind-runtime-packet-20260701.md",
        runner_or_tool="harness/scripts/run-quinn-rebind-runtime-demo.sh",
        local_artifact_id="quinn-rebind-runtime-demo-local-20260701",
        claim_strength="local_runtime_positive",
        supports="Quinn endpoint rebind runtime runner passed: rebind_recv test exit 0 with connected/got-conn/rebound logs, proto migration test exit 0, migration initiated, PATH_CHALLENGE/PATH_RESPONSE counters nonzero, and new path validated.",
        do_not_claim="HTTP/3 application continuity, Chrome/Safari/Android browser handover, managed deployment success, or quic-go-equivalent per-connection active-control behavior.",
        next_gap="Add a custom Quinn HTTP/3 workload only if reviewers require application-layer Rust evidence; otherwise use this as Rust-stack endpoint-rebind runtime evidence.",
    ),
]
