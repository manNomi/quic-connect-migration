...
def test_trial_id_override_derives_matching_artifact_dir() -> None:
    with TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        config = tmp / "controlled-public-origin.env"
        config.write_text(ACTIVE_CONFIG, encoding="utf-8")
        ...
        env.update(
            {
                "CONTROLLED_PUBLIC_CONFIG": config.as_posix(),
                "TRIAL_ID": "controlled-public-chrome-downlink-heartbeat-network-change-001",
                "FINAL_CHROME_NETWORK_CHANGE_RUN_OUTPUT_DIR": (tmp / "out").as_posix(),
                "NETWORK_CHANGE_RUNNER": runner.as_posix(),
                "CHECK_BASELINE_UNLOCK": "0",
                "RUN_POSTCHECKS": "0",
            }
        ...
        assert artifact_dir_out.read_text(encoding="utf-8").strip().endswith(
            "/artifacts/controlled-public-chrome-downlink-heartbeat-network-change-001"
        )
...
def main() -> int:
    ...
    test_trial_id_override_derives_matching_artifact_dir()
