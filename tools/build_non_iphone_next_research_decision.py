...
TRACKS = [
    Track(
        ...
        needed_from_user_or_environment="Refresh AWS credentials, then run the dedicated live NLB+s2n forwarding echo before the packaged rebinding-proxy mode.",
        paper_value="Highest: directly answers the deployment/AWS part of the professor decision without needing iPhone.",
        risk="The first live run only proves forwarding echo and target consistency; the packaged rebinding-proxy mode still must be run separately and does not prove s2n public active migration API support.",
        next_command_or_action="Refresh AWS credentials, run `harness/scripts/run-aws-s2n-nlb-live-data-plane.sh`, then run it again with `PATH_CHANGE_MODE=rebinding_proxy PAYLOAD_CHUNKS=8 CHUNK_DELAY_MS=250` if forwarding echo passes; classify resulting summaries with `tools/classify_aws_s2n_phase2_artifact.py --require-accepted`.",
        supporting_evidence_ids=(
            "s2n-nlb-cid-provider-proof",
            "s2n-nlb-live-readiness",
            "aws-s2n-nlb-live-runner",
            "s2n-active-migration-api-audit",
            "aws-s2n-phase2-rebinding-runner-audit",
            "aws-s2n-phase2-artifact-classifier-contract",
    ...
    Track(
        ...
        current_state="local_controls_pass_public_bridge_gap_origin_and_desktop_path_blocked",
        can_run_now=False,
        blocker="Tracked controlled-public Chrome rows are H3 baseline or negative-control evidence, not CM success; the user-provided public HTTPS origin currently has no `Alt-Svc: h3`, and this host has no active non-iPhone secondary desktop path.",
        needed_from_user_or_environment="Configure the public domain or an AWS public origin with WebPKI TLS, HTTP/3, Alt-Svc, and workload endpoints; connect or enable a non-iPhone secondary desktop path before setting NETWORK_CHANGE_CMD.",
        ...
        next_command_or_action="Prepare public origin, run the controlled public Chrome media/range/upload wrappers, then classify each row with the controlled-public Chrome artifact classifier contract before using it in the paper.",
        supporting_evidence_ids=(
