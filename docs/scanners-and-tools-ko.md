...
Mac+iPhone USB처럼 Wi-Fi를 끄면 뒤늦게 iPhone USB가 default route가 되는 환경은 simultaneous active secondary path가 아니다. 이 경우 명시적으로 latent path-change mode를 허용하고 operator-approved command를 넘겨야 `desktop_path_change_ready` gate가 열린다.

```bash
python3 tools/check_next_final_handover_trial_readiness.py \
  --use-local-config-for-plan \
  --redact-sensitive \
  --check-public-origin \
  --allow-latent-secondary-path \
  --network-change-cmd "networksetup -setairportpower 'en0' off" \
  --output docs/results/final-handover-next-trial-readiness-current-20260629.md
```
...
| desktop path change | no |

active network-change trial에서는 `desktop_path_change_ready`가 required다. strict active secondary path가 있으면 바로 `yes`가 되고, 없더라도 `--allow-latent-secondary-path`와 iPhone USB latent candidate가 함께 있으면 `latent-iphone-usb-failover` mode로 `yes`가 된다. 이 결과는 path-change trigger readiness이지 QUIC CM 성공 evidence가 아니다.
