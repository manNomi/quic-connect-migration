...
| Disk | controlled-public wrapper 기본값 기준 최소 7GiB free space |

server/client/Safari/Android controlled-public wrapper는 시작 시 `ensure-min-disk-free.sh`를 실행한다. 기본 기준은 `MIN_ARTIFACT_FREE_GIB=7`이며, 작은 smoke test에서만 명시적으로 `MIN_ARTIFACT_FREE_GIB=0`을 지정해 우회한다.
