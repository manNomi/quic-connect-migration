...
The Range/resumable-download control refines the download result. Under 6000ms local return-path loss, byte-range no-retry completed only 1/3 runs, while retry2 completed 3/3 runs. Two retry2 rows used one failed byte-range retry and then completed the 1MiB object. However, every completed row was classified as `nat_rebinding_multiple_quic_sessions`, so the safe interpretation is resumable application-level recovery rather than browser single-session CM.
...
- Range/resume source: `docs/results/chrome-h3-rebinding-range-download-control-20260629.md`
