...
| --- | --- | --- | --- | --- | --- | --- |
| `implementation-policy` | implementation | CM primitives exist, but support is implementation- and policy-specific | 50 (PASS=20;PASS_FEASIBILITY=5;PASS_NEGATIVE_CONTROL=25) | 19 (A=16;B=3) | source-backed explanation with repo evidence | Runtime browser and mobile-client migration trials with policy artifacts. |
...
| `silent-client-downlink` | application | Downlink-dominant workloads may not trigger timely migration recovery | 21 (PASS=6;PASS_FEASIBILITY=4;PASS_NEGATIVE_CONTROL=11) | 8 (A=3;B=2;Watch=3) | source-backed explanation with repo evidence | Controlled public active path-change trials with and without heartbeat or retry. |
...
| `performance-risk` | performance | Successful migration can still impose stall, retransmission, or QoE cost | 19 (PASS=9;PASS_FEASIBILITY=4;PASS_NEGATIVE_CONTROL=6) | 6 (B=1;Watch=5) | cautious explanatory support | Measure recovery time, stall duration, and user-visible task success in final browser trials. |
