...
Chapter 1에서 가장 강한 근거는 실제 로컬 테스트를 돌린 8개 구현체다.
...
| MsQuic | source/docs audit | production 중요도는 높지만 별도 로컬 빌드 계획 필요 |
| mvfst | source/docs audit | Folly/Fizz 등 의존성이 커서 source maturity evidence로 유지 |
| lsquic | source/docs audit | LiteSpeed deployment와 연결되지만 optional local test |
| XQUIC | source/docs audit | build/test cost와 docs 언어 장벽 |
| nginx QUIC | source audit | server-side passive migration evidence로 분리 |
| quicly | source audit | 낮은 우선순위의 C implementation 비교군 |
...
Chapter 1에서는 QUIC WG 구현체 목록을 seed로 삼아, 실제 연구와 배포에 영향이 큰 18개 구현체/스택을 골랐다. 각 구현체에 대해 path validation, NAT rebinding, active migration API, migration policy, preferred address, CID/LB, qlog/event, test 존재 여부를 CSV로 정리했다. 이후 keyword scanner로 1차 evidence file을 찾고, source/test를 수동으로 읽어 false positive를 제거했다. 그중 빌드 가능한 8개 구현체는 실제 local test 또는 sample reproduction을 실행했다. 결과적으로 CM은 구현체 수준에서 존재하고 여러 구현체가 L3-L4 이상의 성숙도를 보였지만, 이것이 곧 브라우저나 CDN에서 end-to-end CM이 작동한다는 뜻은 아니므로 Chapter 2에서 deployment/runtime friction을 분석하게 되었다.
