...
상세 결과는 `docs/results/iphone-usb-boundary-and-active-short-20260626.md`와 `docs/results/iphone-usb-trigger-timing-sweep-20260626.md`에 기록했다.

## 다음 실행 추천

가장 가까운 다음 실행은 `boundary8000-32kb application-level recovery`다. no-change baseline은 성공했고 active 2/4/6/7초 trigger는 모두 미완료였으므로, 이제 retry/heartbeat가 사용자 작업 연속성을 회복하는지 별도 축으로 측정하는 것이 좋다.
...
| 8000 | 32768 | 8 | 3 | heartbeat=true diagnostic |
