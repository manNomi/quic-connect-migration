...
| current disk free | `2.7 GiB` |
| target free GiB | `5.0` |
| storage target met by artifact cleanup | `no` |
| remaining external cleanup gap | `1.4 GiB` |
...
| 2 | `todo-now` | storage | Free enough disk before running heavy browser/qlog captures. | Current artifact cleanup candidates are insufficient for the target free-space threshold; remaining external cleanup gap is 1.4 GiB. |
