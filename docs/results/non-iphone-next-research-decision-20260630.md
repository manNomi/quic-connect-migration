...
| 2 | `chrome-controlled-public-workloads`<br>Chrome desktop controlled-public media/range/upload handover | `local_controls_pass_public_bridge_gap_user_origin_not_h3_ready` | `false` | Tracked controlled-public Chrome rows are H3 baseline or negative-control evidence, not CM success; the user-provided public HTTPS origin is reachable but currently has no `Alt-Svc: h3`. | High: bridges local browser evidence to real public-origin web workload continuity without iPhone. | Best browser-facing next step after public origin is available. |
