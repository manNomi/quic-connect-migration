...
| runtime preflight | `harness/scripts/openlitespeed-runtime-preflight.sh` 실행, `runtime_ready=no` |
| latest preflight artifact | `harness/results/openlitespeed-runtime-preflight-20260630T120037Z` |
...
| 아직 증명하지 않은 것 | 이유 |
| --- | --- |
| OpenLiteSpeed binary build success | preflight 결과 disk `19.58GiB < 30GiB`, submodule/binary gate 닫힘 |
...
OpenLiteSpeed는 다음 non-iPhone 연구 단계로 적합하다. 그러나 `docs/results/openlitespeed-runtime-preflight-20260630.md` 기준 현재 macOS 로컬 환경은 `runtime_ready=no`다. 디스크 여유가 `19.58GiB`이고, `src/liblsquic` submodule, `lshttpd` binary, Linux-style `/dev/shm` gate가 닫혀 있으므로 artifact cleanup 또는 Linux/EC2 환경 확보 후 runtime demo로 넘어가는 것이 안전하다.
