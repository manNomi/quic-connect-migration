...
| package path | `harness/results/packages/quic-go-min-repro-20260629T052407Z.tar.gz` |
| package built now | `yes` |
| SSH target placeholder | `ec2-user@<origin-host-or-ip>` |
| remote dir | `/home/ec2-user/quic-go-min-repro` |
| baseline run id | `controlled-public-chrome-downlink-noheartbeat-network-change-001` |
| expected requests | `2` |
| public safe | `yes` |
...
```bash
scp harness/results/packages/quic-go-min-repro-20260629T052407Z.tar.gz ec2-user@<origin-host-or-ip>:/tmp/quic-go-min-repro.tar.gz
