...
| baseline trial | `controlled-public-chrome-range-retry-nochange-fresh-20260629-001` |
| active trials | `controlled-public-chrome-range-retry-network-change-page-ready-fresh-20260629-001` through `003` |
...
| condition | runs | status | classification | qlog path validation | application result |
| --- | ---: | --- | --- | --- | --- |
| no-change baseline | 1 | `PASS` | `controlled_public_application_h3_confirmed` | `false` | `rangeComplete=true`, `rangeRetriesUsed=0` |
| Wi-Fi to iPhone USB | 3 | `PASS_NEGATIVE_CONTROL` | mixed negative controls | `false` in all runs | `2/3` completed |
...
- Successful active runs recorded one transient `TypeError: Failed to fetch`
  and then completed the full `524288` byte task by retrying a byte range.
- The failed active run reached only `262144` bytes and recorded terminal
  `rangeError`.
...
workload-dependent and timing-dependent. A byte-range download can recover from
a path-change interruption through application-level retry/resume even when
transport-level CM is not observed, but only if the replacement path becomes
usable before the retry budget and browser fetch failure behavior terminate the
task. Therefore, the paper should avoid saying "HTTP/3 CM guarantees download
continuity" and instead separate:
