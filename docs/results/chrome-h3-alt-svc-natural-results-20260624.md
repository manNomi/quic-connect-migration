...
> Local self-signed origin에서 Alt-Svc만으로 Chrome natural HTTP/3 upgrade를 재현하지 못했다. 따라서 forced-QUIC browser baseline과 real deployment style browser discovery baseline은 분리해서 검증해야 한다.
...
3. 실제 browser/mobile 연구는 public trusted certificate, Alt-Svc/DNS HTTPS/SVCB, active network change를 포함한 환경에서 다시 수행해야 한다.

## 6. 후속 작업

- public trusted origin 또는 AWS direct-origin HTTPS bootstrap에서 natural HTTP/3 upgrade를 재검증한다.
- Chrome background traffic을 줄이기 위해 추가 flag 또는 isolated test profile 정책을 검토한다.
