...
| Safari baseline/handover result | baseline harness만 있음 | Safari WebDriver wrapper exists; controlled public baseline not run | 부분 충족 |
...
3. CloudFront/Cloudflare 같은 managed CDN edge에서 end-to-end QUIC CM이 성공했다.
4. application heartbeat나 Service Worker가 실제 active path-change recovery를 개선했다.
...
| P1 | Safari controlled public baseline | Safari WebDriver + server qlog + packet capture plan | server/qlog H3 evidence, no NetLog dependency |
| P1 | Android Chrome/Cronet handover | ADB device + Wi-Fi/LTE or tethering path + capture | browser/app policy와 handover behavior 관찰 |
