...
| depth counts | `{'local_test_suite_rerun': 5, 'local_runtime_or_app_demo': 5, 'focused_partial_runtime': 2, 'client_policy_source_plus_local_baseline': 1, 'source_test_map_only': 1, 'managed_or_external_deployment_gate': 2, 'negative_control_runtime': 1}` |
| evidence strength counts | `{'deployment_gate_pending': 2, 'focused_positive_but_not_full_stack': 2, 'negative_control': 1, 'policy_dependent_client_evidence': 1, 'source_level_active_and_passive_evidence': 1, 'strong_implementation_or_runtime_evidence': 5, 'strong_runtime_or_app_evidence': 5}` |
...
| 6 | MsQuic | `local_runtime_or_app_demo` | `strong_runtime_or_app_evidence` | `L4_selected_runtime_tests` | `policy_constrained` | `yes` | `yes` | Selected v4/v6 NAT rebinding and path-validation runtime tests now have a dedicated fail-closed packet, and the focused API audit still shows constrained local-address control plus a QUIC-aware LB boundary rather than quic-go-style AddPath/Probe/Switch control. | Use the MsQuic rebind/path-validation packet as selected runtime-test evidence; build a separate payload-continuity harness or live QUIC-aware LB row only if reviewers require it. |
