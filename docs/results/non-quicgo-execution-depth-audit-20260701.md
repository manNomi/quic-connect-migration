...
| 6 | MsQuic | `local_test_suite_rerun` | `strong_implementation_or_runtime_evidence` | `L4_L5_caveat` | `needs_api_check` | `yes` | `yes` | Selected NAT rebinding and path-validation tests passed, but direct application-triggered active migration and QUIC-aware LB assumptions still need a focused API/deployment check. | Audit the public API path for controlled migration and pair it with a QUIC-aware LB deployment assumption check. |
...
| 9 | XQUIC | `focused_partial_runtime` | `focused_positive_but_not_full_stack` | `L3_L4_partial` | `unclear` | `yes` | `yes` | The NAT rebinding demo passed, but the full suite was deferred to Linux or compiler-flag follow-up because the macOS build path hit unrelated toolchain friction. | Rerun the full test suite on Linux with compatible compiler flags. |
...
| 13 | mvfst | `source_test_map_only` | `source_level_active_and_passive_evidence` | `L5_candidate` | `yes` | `yes` | `yes` | Source/test coverage is strong, but local build/test execution is gated by Buck/getdeps/disk/toolchain cost. | Use a Linux builder with Buck/getdeps and enough disk, then run the focused migration test targets. |
