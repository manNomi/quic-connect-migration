...
Chrome public H3 discovery baseline과 public Alt-Svc target survey 이후, 실제 browser/network-change 실험으로 넘어갈 수 있는지 현재 로컬 장비 상태와 필요한 실험 조건을 점검했다.
...
| 조건 | 필요 이유 |
| --- | --- |
| controlled public WebPKI origin | Chrome workload의 application HTTP/3 처리를 server log/qlog로 증명해야 함 |
...
먼저 no-change application H3 baseline을 통과시킨다.
...
- Chrome target origin이 controlled public application H3 baseline을 통과한 상태
...
3. no-change application H3 browser workload를 먼저 통과시킨다.
4. `check_controlled_public_experiment_readiness.py`에서 `can_run_network_change=true`가 된 뒤에만 handover trigger를 실행한다.
