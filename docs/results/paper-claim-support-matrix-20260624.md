...
| --- | --- | --- | --- | --- | --- |
| `implementation-maturity-is-real-but-heterogeneous` | `supported_scoped` | implementation survey | 18 implementations surveyed; active_migration_api=yes for 8; passive_migration=yes for 14; tests=yes for 14; 14 local test/demo/partial/negative-control artifacts; fresh rerun/demo/negative-control artifacts for 13; LSQUIC preferred-address/NAT-rebinding HTTP/3 app demos; nginx HTTP/3 runtime active-client-migration demo; HAProxy HTTP/3 fresh negative-control; top examples: quic-go, Cloudflare quiche, AWS s2n-quic, ngtcp2, LiteSpeed LSQUIC, nginx QUIC, HAProxy negative-control | QUIC connection migration is not merely a paper feature: surveyed implementations expose path validation, migration or rebinding primitives, tests, and observability at different maturity levels. | Do not claim every HTTP/3 implementation exposes production-ready browser/mobile CM. |
