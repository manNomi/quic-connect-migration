...
| 방법 | 대상 | 확인 항목 |
| --- | --- | --- |
| 소스/문서 검수 | mvfst, Chromium/Cronet, HAProxy 등 | migration API, path validation, NAT rebinding, preferred address, qlog/event, test, 운영 문서 |
| 로컬 실행 검수 | quic-go, Cloudflare quiche, picoquic, s2n-quic, aioquic, ngtcp2, Quinn, Neqo, MsQuic, lsquic, XQUIC, nginx QUIC, HAProxy negative-control, quicly partial | migration test, sample client/server, custom reproduction, qlog/path event |
...
| nginx QUIC | L4 server runtime | 실제 웹 서버 관점에서 quiche active migration을 처리한 runtime positive-control 근거가 있다. |
...
| HAProxy QUIC | L1-L2 negative-control | HTTP/3 proxy baseline은 PASS하지만 active migration path validation은 FAIL인 fresh 반례로 중요하다. |
