...
| LiteSpeed LSQUIC | full CTest 79/79 and selected primitive tests | PASS |
...
LSQUIC은 LiteSpeed/OpenLiteSpeed 생태계와 연결되는 서버 측 구현체다. 이번 보강에서는 source inspection에 머물지 않고, 현재 HEAD 기준으로 full unit suite를 빌드/실행했다.
...
> LSQUIC은 더 이상 source-only 후보가 아니다. 서버 스택 관점의 unit-test maturity evidence는 확보됐다. 다만 이번 검수는 quic-go/quiche처럼 application-level active migration demo가 아니라 full unit suite와 source-linked primitive 근거이므로, OpenLiteSpeed/LSQUIC 기반 실제 HTTP/3 workload migration은 후속 실험으로 남긴다.
