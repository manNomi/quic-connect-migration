...
| quicly e2e path-migration runner | [harness/scripts/run-quicly-e2e-path-migration-check.sh](../../harness/scripts/run-quicly-e2e-path-migration-check.sh) | quicly full e2e 로그에서 `path-migration` subtest만 분리 판정 |
...
quic-go만 강하게 검수했다는 약점을 줄이기 위해 2026-06-30에 10개 구현체를 현재 HEAD 기준으로 다시 실행했고, quicly는 build/unit evidence에 더해 focused e2e `path-migration` subtest evidence를 추가로 확보했다.
...
| MsQuic | `51d449b7d2deb553d6503591f72a8e62d1071054` | build `msquictest`, dedicated runner for selected v4/v6 RebindPort/RebindAddr/PathValidation tests | `validation=ok`, `total_ok_count=8`, `passed_summary_count=2` | `harness/results/msquic-rebind-pathvalidation-local-20260701`, `docs/results/msquic-rebind-pathvalidation-packet-20260701.md` |
| ngtcp2 | `c24b12690c5bdf7ad2715ae427504e76bf5c6ffc` | selected `tests/main` migration/path-validation tests + official `osslclient/osslserver` HTTP/3 runtime migration row | tests `6 of 6 successful`; runtime `validation=ok`, `client_exit=0`, path challenge/response `34/24` | `harness/results/impl-rerun-20260630T070249Z/logs/ngtcp2-migration-tests.log`, `harness/results/ngtcp2-example-migration-demo-local-20260701` |
...
| Quinn | `953b466747e667a9dfda0596b8051a0644f8333d` | `cargo test -p quinn-proto migration`, `cargo test -p quinn rebind`, dedicated endpoint-rebind runtime runner | `1 passed`, `1 passed`, `validation=ok`, PATH_CHALLENGE/PATH_RESPONSE `3/3`, new path validated `1` | `harness/results/impl-rerun-20260630T070249Z/logs/quinn-*.log`, `harness/results/quinn-rebind-runtime-demo-local-20260701` |
...
| quicly | `ed83c7c7d545a01650651c9523466f561ec5d4bb` | build `test.t`/`cli`/`udpfw`, run `test.t`, run `t/e2e.t` and parse `path-migration` | migration-related unit subtests `ok`; focused e2e `path-migration` `ok`; CID seq 1 first path probe check `ok`; full `test.t`/`t/e2e.t` still not clean due unrelated failures | `harness/results/impl-rerun-20260630T070249Z/logs/quicly-*.log`, `harness/results/quicly-e2e-path-migration-local-20260630` |
...
| MsQuic | [GitHub](https://github.com/microsoft/msquic), [official docs](https://microsoft.github.io/msquic/), [deployment docs](https://microsoft.github.io/msquic/msquicdocs/docs/Deployment.html) | selected v4/v6 NAT rebind/path-validation runtime-test evidence with app/LB caveat |
...
| quicly | [GitHub](https://github.com/h2o/quicly) | focused e2e path-migration and path validation/path promotion primitive evidence |
...
| implementation fresh rerun summary | [docs/results/implementation-rerun-results-20260630.md](../results/implementation-rerun-results-20260630.md) | 2026-06-30 현재 HEAD 기준 quiche/picoquic/s2n-quic/MsQuic/LSQUIC/ngtcp2/aioquic/Quinn/Neqo/XQUIC 재실행과 quicly focused e2e 요약 |
| quicly focused e2e path migration | [docs/results/quicly-e2e-path-migration-20260630.md](../results/quicly-e2e-path-migration-20260630.md) | full `t/e2e.t`는 `slow-start` 때문에 FAIL이지만 `path-migration` subtest와 CID seq 1 check는 PASS |
...
| MsQuic selected runtime artifact | `harness/results/msquic-rebind-pathvalidation-local-20260701`, `docs/results/msquic-rebind-pathvalidation-packet-20260701.md` | raw log는 ignored path, public-safe packet은 commit 대상. `validation=ok`, `total_ok_count=8`, `passed_summary_count=2`, `failed_marker_count=0` |
| ngtcp2 fresh artifacts | `harness/results/impl-rerun-20260630T070249Z/logs/ngtcp2-migration-tests.log`, `harness/results/ngtcp2-example-migration-demo-local-20260701` | 현재 로컬에 존재하지만 ignored path라 commit하지 않음 |
...
| Quinn fresh/runtime artifact | `harness/results/impl-rerun-20260630T070249Z/logs/quinn-proto-migration-tests.log`, `harness/results/impl-rerun-20260630T070249Z/logs/quinn-rebind-tests.log`, `harness/results/quinn-rebind-runtime-demo-local-20260701` | 현재 로컬에 존재하지만 ignored path라 commit하지 않음. runtime packet은 `validation=ok`, rebind logs, proto migration, PATH_CHALLENGE/PATH_RESPONSE, new path validated count를 요약함 |
...
| quicly focused e2e artifact | `harness/results/quicly-e2e-path-migration-local-20260630` | 현재 로컬에 존재하지만 ignored path라 commit하지 않음. `validation=ok_path_migration`, `path_subtest_ok=yes`, full e2e는 `slow-start` caveat |
...
quicly focused e2e path-migration check:

```bash
RUN_ID=quicly-e2e-path-migration-$(date -u +%Y%m%dT%H%M%SZ) \
...
| quic-go 외 구현체도 fresh rerun/runtime 근거를 확보했는가? | PASS | quiche, picoquic, s2n-quic, MsQuic selected runtime packet, LSQUIC, nginx QUIC, ngtcp2, aioquic, Quinn endpoint-rebind runtime, Neqo, XQUIC NAT rebinding demo |
...
| partial/focused evidence를 분리했는가? | PASS | quicly는 focused e2e `path-migration` evidence와 full e2e caveat를 분리 |
