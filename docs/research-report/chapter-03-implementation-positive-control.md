...
2026-06-30 추가 재검수에서는 quiche, picoquic, s2n-quic, MsQuic, ngtcp2, aioquic, Quinn, Neqo도 현재 HEAD 기준으로 다시 실행해 PASS evidence를 확보했고, XQUIC은 NAT rebinding demo PASS evidence를 확보했다. quicly는 build와 migration-related unit evidence를 확보했지만 full test/e2e는 partial로 분리했다. 다만 이 결론은 library/client-server positive control이다. Chrome/Safari/Android browser HTTP/3 handover 성공을 의미하지 않는다.
...
| “quiche/picoquic/s2n-quic raw log가 공개 repo에 커밋되어 있다.” | 공개 repo에는 요약 문서와 재현 명령이 있고, raw log는 로컬 ignored path에 있다. |
