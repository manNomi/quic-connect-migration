...
## 7.1 Non-iPhone Local Range Refresh

public handover와 별도로, iPhone 없이 local Chrome forced-H3 byte-range control도 fresh로 2회 실행했다.

| run | retry | status | classification | complete | elapsed ms | Chrome sessions | remote tuples | qlog C/R |
| --- | ---: | --- | --- | --- | ---: | ---: | ---: | --- |
| `chrome-desktop-noniphone-range-drop3000-retry0-20260630` | 0 | PASS | `nat_rebinding_possible_session_continuity` | true | 1095 | 1 | 2 | 1/1 |
| `chrome-desktop-noniphone-range-slow-drop3000-retry0-20260630` | 0 | PASS | `nat_rebinding_possible_session_continuity` | true | 6122 | 1 | 2 | 1/1 |

이 local refresh는 public iPhone handover 결과와 해석층이 다르다. public active path-change rows에서는 qlog path validation이 없었고 retry가 task completion을 회복했다. 반면 local UDP rebinding rows에서는 qlog path validation과 single target session이 관찰됐다. 따라서 논문에서는 두 결과를 합쳐 "환경에 따라 browser artifact가 크게 달라진다"는 근거로 쓰고, public handover success로 섞지 않는다.
