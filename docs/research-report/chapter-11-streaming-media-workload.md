...
| 6000ms fresh | 0 | 1/1 | 1/1 | 1/1 | 23 | 14058 | 6 | 3 | `nat_rebinding_multiple_quic_sessions` |
| 6000ms fresh | 0 | 4/6 | 1/1 | 1/1 | 22 | 9043 | 1 | 3 | `nat_rebinding_multiple_quic_sessions` |
...
2026-07-01 fresh non-iPhone buffered-media refresh에서는 더 긴 6000ms A+B return-path outage를 low/high buffer 1회씩 재검수했다. 두 row 모두 playback은 완료됐지만 low buffer는 rebuffer 6회, high buffer는 rebuffer 1회였고, 두 row 모두 Chrome target QUIC session 3개였다. 따라서 동영상형 workload에서는 completion, startup/rebuffer, session churn을 함께 보고해야 한다.

따라서 streaming 연구에서는 "재생 완료" 하나만으로는 부족하다.

2026-07-01 non-iPhone workload continuity/QoE synthesis는 이 결론을 upload/range까지 묶어서 다시 정규화했다. 기존 local Chrome/quic-go CSV 8개에서 32개 row를 모았을 때, byte-range download는 2/2 completion과 2/2 single-session path-validation row를 보였고 upload도 1/1 single-session path-validation row를 보였다. 반면 buffered video는 14/14 playback complete였지만 14/14가 multiple Chrome target sessions였고, music-like segment는 retry0 0/4 실패, retry1 4/4 완료로 갈라졌다. 따라서 다음 public-origin trial은 range/upload처럼 gate가 선명한 workload를 먼저 실행하고, streaming은 startup delay, rebuffer, retry, session churn을 함께 보고하는 방식으로 설계해야 한다.
