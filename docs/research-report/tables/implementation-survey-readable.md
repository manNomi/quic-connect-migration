...
| 5 | LiteSpeed lsquic | server | O | O | O | `logs` | O | `likely` | `L4_L5_candidate` | `fresh_rerun_20260630` | Use as server-stack unit evidence; still build app-level OpenLiteSpeed/lsquic migration harness if needed |
...
| LiteSpeed lsquic | full CTest 79/79와 migration/preferred-address setting 근거를 확보함 |
...
| LiteSpeed lsquic | 서버 스택 단위 테스트와 migration 설정 근거가 있어 OpenLiteSpeed/서버 배포 논의에 연결 가능 |
...
2. active migration API가 명확한 구현체는 더 적지만, quic-go/quiche/picoquic/Neqo 등에서 실험 후보가 확인됐고 MsQuic과 LSQUIC은 각각 selected gtest와 full CTest 근거가 보강됐다. quicly는 partial build/test evidence로 분리한다.
