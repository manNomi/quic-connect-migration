...
15. local UDP rebinding proxy 반복 실험에서는 no-heartbeat 3회 모두 qlog path validation은 관찰됐지만 request-level remote tuple은 하나로 남았다. heartbeat 3회 모두 server remote tuple 2개와 qlog path validation이 관찰됐지만 Chrome target QUIC session도 2개였으므로 browser session continuity claim으로 쓰면 안 된다.
16. local UDP rebinding streaming upload 반복 실험에서는 3회 모두 256KiB upload가 완료됐고 qlog path validation 및 단일 Chrome target QUIC session이 관찰됐다. 그러나 request-level remote tuple은 하나로 남았으므로 request log만으로 packet-level rebinding 여부를 판단하면 안 된다.
