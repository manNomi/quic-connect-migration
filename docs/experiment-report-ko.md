...
13. public WebPKI origin에서는 같은 Chrome 149 headless가 forced QUIC 없이 natural HTTP/3를 사용했다. Cloudflare QUIC trace endpoint와 Google `generate_204` endpoint 모두 target `QUIC_SESSION`과 `HTTP_STREAM_JOB using_quic=true` evidence가 있었다.
14. public Alt-Svc survey에서는 Google/Cloudflare/YouTube 계열 endpoint가 H3 후보였고, GitHub/Naver/Kakao는 이번 관찰에서 H3 Alt-Svc를 광고하지 않았다. Amazon은 H3 Alt-Svc는 있었지만 `HTTP/2 503`이라 workload target으로 부적합했다.
...
> Chrome은 HTTP/3를 사용할 수 있다. 다만 local self-signed/mkcert Alt-Svc 실험은 public deployment style browser discovery를 대체하지 못한다.
...
7. public WebPKI natural H3 baseline이 성공했으므로 Chrome browser network change에서도 곧바로 Connection Migration이 성공한다고 말한다.
...
- public WebPKI Cloudflare QUIC trace endpoint에서는 forced QUIC 없이 target `QUIC_SESSION=1`과 `using_quic` job이 관찰됐음
...
- third-party public H3 endpoint는 discovery positive control에는 충분하지만 upload/download/dashboard workload continuity 실험에는 controlled public origin이 필요하다.
