...
12. Chrome natural Alt-Svc control에서는 local self-signed origin이 h3를 광고했지만, forced QUIC 없이 실제 HTTP/3 application request가 관찰되지는 않았다. HTML diagnostic에서는 QUIC/H3 후보 연결이 열렸지만 `certificate unknown / CERTIFICATE_VERIFY_FAILED`로 닫혔다.
