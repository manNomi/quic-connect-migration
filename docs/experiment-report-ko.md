...
12. Chrome natural Alt-Svc control에서는 local self-signed origin이 h3를 광고했지만, forced QUIC 없이 실제 HTTP/3 request가 server protocol record나 qlog에서 관찰되지는 않았다.
...
- `run-chrome-h3-alt-svc.sh` natural Alt-Svc control에서 `127.0.0.1`과 `localhost` 모두 server request는 `HTTP/1.1`로만 기록됐고 qlog `http3_frame=0`이었음
...
- 또한 browser가 target origin을 실제 HTTP/3로 선택했다는 evidence를 먼저 확보해야 한다. local self-signed Alt-Svc control에서는 h3 광고만으로 충분하지 않았다.
