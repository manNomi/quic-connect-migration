...
13. public WebPKI origin에서는 같은 Chrome 149 headless가 H3 discovery 또는 QUIC session 단서를 만들었다. 그러나 재분류 결과 Cloudflare, Google, YouTube 모두 application/main request job은 non-QUIC였고, `using_quic=true` job은 `dns_alpn_h3` discovery job으로 분리됐다. 따라서 제3자 public endpoint NetLog만으로 application HTTP/3 사용을 확정하지 않는다.
...
> Chrome은 forced local baseline에서 HTTP/3 application request를 보낼 수 있다. 다만 local self-signed/mkcert Alt-Svc 실험과 third-party public discovery evidence는 controlled public application H3 baseline을 대체하지 못한다.
