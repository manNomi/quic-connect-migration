...
구현체 primitive 존재 확인
  ...
  -> Chrome browser H3 baseline, downlink/heartbeat workload, local UDP rebinding proxy, multiple-session negative control
```
...
마지막으로 transient return-path outage sweep을 추가했다. A+B server-to-client packet drop을 250ms, 1500ms, 3000ms, 4000ms, 5000ms, 6000ms, 9000ms window로 제한했고, 1MiB downlink/upload 기준 250ms/1500ms/3000ms/4000ms는 8/8 PASS, 5000ms/6000ms/9000ms는 6/6 FAIL이었다. PASS row는 DOM complete timing을 7545-11309ms 범위로 남겼고, FAIL row는 DOM error timing을 6918-11122ms 범위로 남겼다. 이 결과는 local NAT rebinding harness에서의 outage-tolerance boundary이며, 실제 Wi-Fi/LTE handover 성공으로 일반화하지 않는다.

이 경계를 반복 검수하기 위해 4000ms, 4500ms, 5000ms window를 downlink/upload 각각 3회씩 다시 실행했다. 4000ms와 4500ms는 각각 6/6 PASS였고, 5000ms는 downlink 3/3 PASS였지만 upload 3/3 FAIL이었다. 전체 18개 row 중 15개가 완료됐고, 실패 3개는 모두 5000ms upload였다. 따라서 5초 근처의 outage tolerance는 단일 threshold가 아니라 workload-sensitive transition zone으로 해석해야 한다. 특히 5초 upload 실패 row도 qlog H3/path evidence와 proxy switch evidence를 남겼으므로, transport evidence와 application completion의 분리는 반복 실험에서도 유지됐다.

Downlink 쪽 transition을 더 좁히기 위해 5000ms/5500ms/6000ms window를 downlink-only로 각각 3회 실행했다. 5000ms와 5500ms는 각각 2/3 PASS로 혼재했고, 6000ms는 3/3 FAIL이었다. PASS row의 DOM complete timing은 12276-14114ms였고, FAIL row의 DOM error timing은 6922-6927ms였다. 모든 row가 qlog H3/path evidence를 남겼으므로, downlink도 5초대에서는 packet/timer alignment에 민감한 transition zone이며, 6초에서는 반복 실패 구간으로 관찰됐다.

5000ms에서 upload만 반복 실패했기 때문에 upload-only fine boundary도 수행했다. 4600ms, 4750ms, 4900ms, 5000ms window를 upload workload로 각각 3회 실행한 결과, 4600ms는 3/3 PASS, 4750ms는 1/3 PASS, 4900ms와 5000ms는 6/6 FAIL이었다. PASS row의 DOM complete timing은 10215-11261ms였고 FAIL row의 DOM error timing은 6917-6922ms였다. 따라서 이 local 1MiB upload workload에서는 안정 완료 구간이 4600ms까지, 불안정 transition이 4750ms부터, 반복 실패 구간이 4900ms부터 관찰됐다.

같은 4900ms/5000ms upload 실패 구간에서 application-level retry control도 실행했다. 브라우저 upload page가 첫 `fetch()` 실패 후 1000ms 뒤 새 `ReadableStream`으로 한 번 재시도하도록 설정하자 4900ms 3/3, 5000ms 3/3, 총 6/6이 PASS였다. 모든 row는 `/upload-sink` request 2개와 최종 1MiB 수신을 기록했고, qlog H3/path evidence도 남겼다. 그러나 모든 row의 Chrome target QUIC session count는 2였고 classification은 `nat_rebinding_multiple_quic_sessions`였다. 즉 application-level retry는 사용자 작업 완료를 회복할 수 있지만, 이 회복은 single-session browser CM 성공과 다르게 분류해야 한다.

동일한 retry control을 6000ms/9000ms 장기 outage로 확장했을 때도 6/6이 PASS였다. 6000ms row는 약 15.5초에 완료됐고, 9000ms row는 약 19.7초에 완료됐다. 그러나 Chrome target QUIC session count는 2-3개였으므로, 긴 outage에서 retry는 task completion을 회복하는 대신 completion latency와 replacement/multiple-session behavior를 동반한다고 해석해야 한다.

마지막으로 12000ms/15000ms retry stress boundary를 수행했다. 12000ms upload는 3/3 PASS였고 약 20.0초에 완료됐다. 반면 15000ms upload는 3/3 FAIL이었고 DOM error timing은 15936-15943ms였다. 실패 row는 server request count가 2에 그쳤고 두 번째 `/upload-sink`가 도달하지 못했으며 upload bytes는 0이었다. qlog H3/path evidence와 Chrome target QUIC session count 3은 여전히 관찰됐으므로, transport/browser evidence가 있어도 application-level retry recovery는 12-15초 사이에서 깨질 수 있다.

그 다음 동일한 15000ms outage에서 application-level retry를 2회로 늘려 재검수했다. 3회 모두 PASS였고 최종 1MiB upload가 완료됐지만 DOM complete timing은 24484-24503ms였으며 Chrome target QUIC session count는 4개였다. 따라서 retry budget을 늘리면 15초 outage도 task completion까지 회복할 수 있으나, 이는 더 긴 사용자 대기 시간과 더 많은 replacement/multiple QUIC session을 동반하는 application-level recovery이지 single-session browser CM success가 아니다.

2회 retry recovery의 failure side를 찾기 위해 18000ms/21000ms outage도 반복했다. 18000ms upload는 3/3 PASS였고 DOM complete timing은 28196-28199ms였다. 21000ms upload는 3/3 FAIL이었고 DOM error timing은 20950-20955ms였다. 모든 row는 qlog H3/path evidence와 Chrome target QUIC session count 4를 남겼지만, 21000ms 실패 row는 `/upload-sink`가 한 번만 서버에 도달했고 upload bytes는 0이었다. 따라서 2회 retry의 local recovery boundary는 18-21초 사이로 관찰됐다.

마지막으로 no-retry, 1회 retry, 2회 retry upload boundary control을 `Application Recovery Tradeoff` 표로 종합했다. 최신 all-pass window는 각각 4600ms, 12000ms, 18000ms로 이동했고, 해당 window의 DOM complete timing은 약 10.2초, 20.0초, 28.2초로 증가했다. Chrome target QUIC session count도 1개, 3개, 4개로 증가했다. 따라서 작업 완료율만 보면 retry가 좋아 보이지만, 논문에서는 recovery latency와 browser session continuity cost를 함께 보고해야 한다.
...
| MsQuic | selected v4/v6 rebind/path-validation runtime-test packet PASS, constrained local-address API, QUIC-aware LB caveat | production-relevant implementation positive control |
...
15. local UDP rebinding proxy 반복 실험에서는 no-heartbeat 3회 모두 proxy packet log상 A/B upstream forwarding과 qlog/Chrome target NetLog path validation은 관찰됐지만 request-level remote tuple은 하나로 남았다. heartbeat 3회 모두 packet rebinding, server remote tuple 2개, qlog/NetLog target path validation이 관찰됐지만 Chrome target QUIC session도 2개였으므로 browser session continuity claim으로 쓰면 안 된다.
16. local UDP rebinding streaming upload 반복 실험에서는 3회 모두 proxy packet log상 A/B upstream forwarding이 있었고 256KiB upload가 완료됐으며 qlog/Chrome target NetLog path validation 및 단일 Chrome target QUIC session이 관찰됐다. 그러나 request-level remote tuple은 하나로 남았으므로 request log만으로 packet-level rebinding 여부를 판단하면 안 된다.
17. public Alt-Svc survey에서는 20개 endpoint 중 12개가 H3 Alt-Svc를 광고했고, 그중 7개가 2xx workload 후보였다. GitHub/Naver/Kakao/Apple/Wikipedia/Microsoft/Netflix/TikTok은 이번 관찰에서 H3 workload 후보가 아니었다.
18. local transient return-path outage sweep에서는 250ms/1500ms/3000ms/4000ms A+B return-path loss가 1MiB downlink/upload 8/8 PASS였지만, 5000ms/6000ms/9000ms는 6/6 FAIL이었다.
19. local transient boundary repetition에서는 4000ms/4500ms A+B return-path loss가 12/12 PASS였고, 5000ms는 downlink 3/3 PASS와 upload 0/3 PASS로 갈렸다.
20. downlink-only fine boundary에서는 5000ms/5500ms가 각각 2/3 PASS로 혼재했고, 6000ms가 3/3 FAIL이었다.
21. upload-only fine boundary에서는 4600ms가 3/3 PASS, 4750ms가 1/3 PASS, 4900ms/5000ms가 6/6 FAIL이었다.
22. 같은 4900ms/5000ms upload 실패 구간에서 application-level retry 1회를 허용하면 6/6 PASS였지만, 모두 Chrome target QUIC session 2개로 분류되어 retry/reconnect 기반 task recovery와 single-session CM success를 분리해야 함을 확인했다.
23. 6000ms/9000ms 장기 outage에서도 retry는 6/6 PASS였지만, Chrome target QUIC session count가 2-3개로 관찰되어 recovery latency와 session continuity를 별도 결과로 보고해야 함을 확인했다.
24. 12000ms/15000ms retry stress boundary에서는 12000ms가 3/3 PASS, 15000ms가 3/3 FAIL이어서 1회 retry recovery도 무제한 보장이 아니며 local boundary가 12-15초 사이임을 확인했다.
25. 15000ms outage에서 application-level retry를 2회로 늘리면 3/3 PASS였지만 DOM complete timing은 약 24.5초이고 Chrome target QUIC session count는 4개였으므로, recovery budget 증가는 작업 완료를 회복하는 대신 latency와 session churn을 키운다는 점을 확인했다.
26. 18000ms/21000ms retry2 stress boundary에서는 18000ms가 3/3 PASS, 21000ms가 3/3 FAIL이어서 2회 retry recovery도 18-21초 사이에서 다시 깨진다는 점을 확인했다.
27. Application recovery tradeoff 표는 retry budget이 boundary를 오른쪽으로 이동시키지만 DOM completion latency와 Chrome session churn도 함께 증가한다는 논문용 요약 근거를 제공한다.
...
- [Chrome local UDP rebinding upload 반복 결과](results/chrome-h3-rebinding-upload-summary-20260624.md)
