...
20. upload-only fine boundary에서는 4600ms가 3/3 PASS, 4750ms가 1/3 PASS, 4900ms/5000ms가 6/6 FAIL이었다.
21. 같은 4900ms/5000ms upload 실패 구간에서 application-level retry 1회를 허용하면 6/6 PASS였지만, 모두 Chrome target QUIC session 2개로 분류되어 retry/reconnect 기반 task recovery와 single-session CM success를 분리해야 함을 확인했다.
22. 6000ms/9000ms 장기 outage에서도 retry는 6/6 PASS였지만, Chrome target QUIC session count가 2-3개로 관찰되어 recovery latency와 session continuity를 별도 결과로 보고해야 함을 확인했다.
23. 12000ms/15000ms retry stress boundary에서는 12000ms가 3/3 PASS, 15000ms가 3/3 FAIL이어서 1회 retry recovery도 무제한 보장이 아니며 local boundary가 12-15초 사이임을 확인했다.
24. 15000ms outage에서 application-level retry를 2회로 늘리면 3/3 PASS였지만 DOM complete timing은 약 24.5초이고 Chrome target QUIC session count는 4개였으므로, recovery budget 증가는 작업 완료를 회복하는 대신 latency와 session churn을 키운다는 점을 확인했다.
25. 18000ms/21000ms retry2 stress boundary에서는 18000ms가 3/3 PASS, 21000ms가 3/3 FAIL이어서 2회 retry recovery도 18-21초 사이에서 다시 깨진다는 점을 확인했다.
26. Application recovery tradeoff 표는 retry budget이 boundary를 오른쪽으로 이동시키지만 DOM completion latency와 Chrome session churn도 함께 증가한다는 논문용 요약 근거를 제공한다.
