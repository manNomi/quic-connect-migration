...
| GET | `/browser-media-segments` | Chrome segment-based media workload용 HTML/JS 생성 |
| GET | `/browser-buffered-media` | bounded buffer와 playback clock이 있는 Chrome media workload용 HTML/JS 생성 |
| GET | `/media-segment` | video/music-like segment binary response 반환 |
| GET | `/browser-range-download` | Chrome byte-range resumable download workload용 HTML/JS 생성 |
| GET | `/range-download` | `Range: bytes=start-end`를 처리하는 binary response 반환 |
...
| GET | `/browser-downlink` | Chrome downlink-dominant workload용 HTML/JS 생성 |
| GET | `/downlink-stream` | chunk delay가 있는 streaming binary response |
| GET | `/heartbeat` | downlink workload 중 application-level heartbeat request |
...
| `downlink` | `GET /browser-downlink`, streaming `GET /downlink-stream`, optional `GET /heartbeat` |
| `media` | `GET /browser-media-segments`, repeated `GET /media-segment` fetch requests |
| `buffered-media` | `GET /browser-buffered-media`, buffered playback over repeated `GET /media-segment` fetch requests |
| `range` | `GET /browser-range-download`, repeated `GET /range-download` byte-range fetch requests |
